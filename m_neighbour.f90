...
contains
  ...
  function compute_binned_pbc( nat, ityp, pos, lat, rcut )result(self)
    ...
    ! type( t_binned_neighbour ), pointer :: self
    ...
    ! all atoms need to be inside box
    ...
    do i = 1, nat
       ...
       ! low-end buffer
       ! low x
       if( r(1) .le. lo_buffer ) then
          ! add to high-end
          rf = r + [ 1.0_rp, 0.0_rp, 0.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, ibin(2), ibin(3))% bin_add( 2, i, rf )
       end if
       ! low y
       if( r(2) .le. lo_buffer ) then
          ! add to high-end
          rf = r + [ 0.0_rp, 1.0_rp, 0.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( ibin(1), nbins, ibin(3))% bin_add( 3, i, rf )
       end if
       ! low z
       if( r(3) .le. lo_buffer ) then
          ! add to high-end
          rf = r + [ 0.0_rp, 0.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( ibin(1), ibin(2), nbins)% bin_add( 4, i, rf )

       end if
       ! low x and y
       if( r(1) .le. lo_buffer .and. &
            r(2) .le. lo_buffer ) then
          ! add to high-end
          rf = r + [1.0_rp, 1.0_rp, 0.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, nbins, ibin(3) )% bin_add( 5, i, rf )
       end if
       ! low x and z
       if( r(1) .le. lo_buffer .and. &
            r(3) .le. lo_buffer ) then
          ! add to high-end
          rf = r + [1.0_rp, 0.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, ibin(2), nbins )% bin_add( 6, i, rf )
       end if
       ! low y and z
       if( r(2) .le. lo_buffer .and. &
            r(3) .le. lo_buffer ) then
          ! add to high-end
          rf = r + [0.0_rp, 1.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( ibin(1), nbins, nbins )% bin_add( 7, i, rf )
       end if
       ! low x, y and z
       if( r(1) .le. lo_buffer .and. &
            r(2) .le. lo_buffer .and. &
            r(3) .le. lo_buffer ) then
          ! add to high-end
          rf = r + [1.0_rp, 1.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, nbins, nbins )% bin_add( 8, i, rf )
       end if
       ...
       ! high-end buffer
       ! high x
       if( r(1) .ge. hi_buffer ) then
          ! add to low-end
          rf = r - [1.0_rp, 0.0_rp, 0.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, ibin(2), ibin(3))% bin_add( 2, i, rf )
       end if
       ! high y
       if( r(2) .ge. hi_buffer ) then
          ! add to low-end
          rf = r - [0.0_rp, 1.0_rp, 0.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( ibin(1), 1, ibin(3))% bin_add( 3, i, rf )
       end if
       ! high z
       if( r(3) .ge. hi_buffer ) then
          ! add to low-end
          rf = r - [0.0_rp, 0.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( ibin(1), ibin(2), 1)% bin_add( 4, i, rf )
       end if
       ! high x and y
       if( r(1) .ge. hi_buffer .and. &
            r(2) .ge. hi_buffer ) then
          ! add to low-end
          rf = r - [1.0_rp, 1.0_rp, 0.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, 1, ibin(3))% bin_add( 5, i, rf )
       end if
       ! high x and z
       if( r(1) .ge. hi_buffer .and. &
            r(3) .ge. hi_buffer ) then
          ! add to low-end
          rf = r - [1.0_rp, 0.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, ibin(2), 1 )% bin_add( 6, i, rf )
       end if
       ! high y and z
       if( r(2) .ge. hi_buffer .and. &
            r(3) .ge. hi_buffer ) then
          ! add to low-end
          rf = r - [0.0_rp, 1.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( ibin(1), 1, 1 )% bin_add( 7, i, rf )
       end if
       ! high x, y and z
       if( r(1) .ge. hi_buffer .and. &
            r(2) .ge. hi_buffer .and. &
            r(3) .ge. hi_buffer ) then
          ! add to low-end
          rf = r - [1.0_rp, 1.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, 1, 1 )% bin_add( 8, i, rf )
       end if

       ! high-low mixed buffer
       ! high x low y
       if( r(1) .ge. hi_buffer .and. &
            r(2) .le. lo_buffer ) then
          rf = r + [ -1.0_rp, 1.0_rp, 0.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, nbins, ibin(3) )% bin_add( 9, i, rf )
       end if
       ! high x low z
       if( r(1) .ge. hi_buffer .and. &
            r(3) .le. lo_buffer ) then
          rf = r + [ -1.0_rp, 0.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, ibin(2), nbins )% bin_add( 9, i, rf )
       end if
       ! high y low x
       if( r(2) .ge. hi_buffer .and. &
            r(1) .le. lo_buffer ) then
          rf = r + [ 1.0_rp, -1.0_rp, 0.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, 1, ibin(3) )% bin_add( 9, i, rf )
       end if
       ! high y low z
       if( r(2) .ge. hi_buffer .and. &
            r(3) .le. lo_buffer ) then
          rf = r + [ 0.0_rp, -1.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( ibin(1), 1, nbins )% bin_add( 9, i, rf )
       end if
       ! high z low x
       if( r(3) .ge. hi_buffer .and. &
            r(1) .le. lo_buffer ) then
          rf = r + [ 1.0_rp, 0.0_rp, -1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, ibin(2), 1 )% bin_add( 9, i, rf )
       end if
       ! high z low y
       if( r(3) .ge. hi_buffer .and. &
            r(2) .le. lo_buffer ) then
          rf = r + [ 0.0_rp, 1.0_rp, -1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( ibin(1), nbins, 1 )% bin_add( 9, i, rf )
       end if
       ...
       if( r(1) .ge. hi_buffer .and. &
          ...
          rf = r + [ -1.0_rp, 1.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, nbins, nbins )% bin_add( 9, i, rf )
       end if
       ! low x high y low z
       if( r(1) .le. lo_buffer .and. &
            r(2) .ge. hi_buffer .and. &
          ...
          rf = r + [ 1.0_rp, -1.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, 1, nbins )% bin_add( 9, i, rf )
       end if
       ! low x low y high z
       if( r(1) .le. lo_buffer .and. &
            r(2) .le. lo_buffer .and. &
          ...
          rf = r + [ 1.0_rp, 1.0_rp, -1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, nbins, 1 )% bin_add( 9, i, rf )
       end if
       ...
       if( r(1) .ge. hi_buffer .and. &
          ...
          rf = r + [ -1.0_rp, -1.0_rp, 1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, 1, nbins )% bin_add( 9, i, rf )
       end if
       ! high x low y high z
       if( r(1) .ge. hi_buffer .and. &
            r(2) .le. lo_buffer .and. &
          ...
          rf = r + [ -1.0_rp, 1.0_rp, -1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( 1, nbins, 1 )% bin_add( 9, i, rf )
       end if
       ! low x high y high z
       if( r(1) .le. lo_buffer .and. &
            r(2) .ge. hi_buffer .and. &
          ...
          rf = r + [ 1.0_rp, -1.0_rp, -1.0_rp ]
          call crist_to_cart( rf, lat, invlat )
          call self% bins( nbins, 1, 1 )% bin_add( 9, i, rf )
       end if
    ...
    do i = 1, nat
       ...
       do jj = jj_lb, jj_ub
          do kk = kk_lb, kk_ub
             do ll = ll_lb, ll_ub
       ! do jj = 1-nbins_check, 1+nbins_check
       !    do kk = 1-nbins_check, 1+nbins_check
       !       do ll = 1-nbins_check, 1+nbins_check
                ! xbin =  mod( ibin(1) + nbins - jj, nbins ) + 1
                ! ybin =  mod( ibin(2) + nbins - kk, nbins ) + 1
                ! zbin =  mod( ibin(3) + nbins - ll, nbins ) + 1
  ...
  subroutine add( self, origin_idx, idx, vec )
    ...
    if( any(self% neiglist(i_s:self% head_idx) .eq. idx) )then
       ...
       ! the periodic image as its own neighbor
  ...
  function get_list( self, idx, list )result(n)
    !! Return the list of indices from neiglist, which are neighbours of idx.
    !! The index idx is NOT included in the returned array.
  ...
  function get_veclist( self, idx, veclist )result(n)
    !! Return vectors from veclist, which are neighbors of idx.
